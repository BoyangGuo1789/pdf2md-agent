...
PDF 转 Markdown 天然有信息损失。阅读顺序、表格、公式、图片、扫描页都可能悄悄出错。`pdf2md-agent` 默认生成适合 LLM 阅读的 Markdown，同时保留来源哈希、页码锚点、页面图片、结构化 JSON 和验证日志，让下游 Agent 可以回溯证据页并识别高风险转换结果。
...
面向开源社区传播的长版中文介绍见 [pdf2md_agent_open_source_intro.md](pdf2md_agent_open_source_intro.md)。
